...
const StarShipsList = ({ response, fetchStarships }) => {
    ...
    const renderPreviousBtn = (prelink) => {
        if (!prelink)
            return <Button content='Anterior' icon='left arrow' labelPosition='left' disabled />;

        return <Button content='Anterior' icon='left arrow' labelPosition='left' onClick={() => handlePageLink(prelink)} />;
    ...
    const renderNextBtn = (nextlink) => {
        if (!nextlink)
            return <Button content='Próxima' icon='right arrow' labelPosition='right' disabled />;

        return <Button content='Próxima' icon='right arrow' labelPosition='right' onClick={() => handlePageLink(nextlink)} />;
    }
